...
UI::UI() {
  ...
  std::cout.flush();
  this->map_ = generator.Generate(RenderEngine::Instance().GetWidth(), RenderEngine::Instance().GetHeight(), 1, NULL);
...
Player* UI::GetPlayer() { return player_; }

Monster* UI::GetMonster() { return monster_; }

void UI::SetState(GameState state) { state_ = state; }
Map* UI::GetMap() { return map_; }
